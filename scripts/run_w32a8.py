...
from src.quantization.ptq.activation_only_ptq import apply_activation_only_quantization
...
def main():
    ...
    # W32A8
    # -----------------------------------
    print("\nApplying W32A8 quantization...")

    quantized_model = apply_activation_only_quantization(
        model, calib_loader, num_bits=8
    )

    # -----------------------------------
    # Evaluate
    # -----------------------------------
    print("\nEvaluating W32A8 model...")

    quant_acc = evaluate(quantized_model, val_loader, device)

    print(f"W32A8 Accuracy: {quant_acc * 100:.2f}%")
    ...
    result_file = os.path.join(save_dir, f"{args.model}_{args.dataset}_w32a8.txt")

    with open(result_file, "w") as f:

        f.write(f"FP32,{fp32_acc * 100:.2f}\n")
        f.write(f"W32A8,{quant_acc * 100:.2f}\n")
