...
def main():
    ...
    # Apply AdaRound INT4
    print("\nApplying AdaRound INT4...")
    model = apply_adaround(model, train_loader, device, num_bits=4)
    ...
    print("\nEvaluating INT4 quantized model...")
    acc = evaluate(model, test_loader, device)

    print(f"\nAdaRound INT4 Accuracy: {acc*100:.2f}%")

    # Save results
    os.makedirs("results/ptq_results", exist_ok=True)

    with open("results/ptq_results/adaround_int4_results.txt", "a") as f:
