...
def main():
    ...
    # -----------------------------------
    # Progressive cumulative quantization
    # -----------------------------------
    for i in range(len(target_blocks)):

        current_blocks = target_blocks[: i + 1]

        print("\n================================================")
        print(f"[RUNNING] Quantizing blocks:")
        print(current_blocks)
        print("================================================")

        analyzer = ActivationSensitivityAnalyzer(
            model=model, target_module_names=current_blocks
        )
        ...
        results.append(
            {
                "num_quantized_blocks": len(current_blocks),
                "last_quantized_block": current_blocks[-1],
    ...
    save_path = os.path.join(
        output_dir, f"{args.model}_{args.dataset}_cumulative_activation_sensitivity.csv"
    )
