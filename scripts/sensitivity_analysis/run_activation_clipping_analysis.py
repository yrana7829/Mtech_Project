...
from src.quantization.sensitivity.activation_clipping import ActivationClippingAnalyzer
...
def main():
    ...
    # Calibration ranges
    # -------------------------
    print("\nCollecting calibration ranges...")

    calibration_ranges = collect_activation_ranges(model, calib_loader)

    # -------------------------
    # Clipping analyzer
    # -------------------------
    analyzer = ActivationClippingAnalyzer(target_layers, calibration_ranges)
    ...
    save_path = os.path.join(
        save_dir, f"{args.model}_" f"{args.dataset}_" f"activation_clipping.csv"
    )
